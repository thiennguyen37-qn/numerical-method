...
% Logo truong -- cap nhat duong dan anh sau
\includegraphics[width=3cm]{qnu_logo.jpg}\\[0.8cm]

{\large\bfseries TRƯỜNG ĐẠI HỌC QUY NHƠN}\\[0.5cm]

{\normalsize\bfseries BÀI TIỂU LUẬN CUỐI KỲ HỌC PHẦN}\\[0.15cm]
{\normalsize\bfseries PHƯƠNG PHÁP SỐ CHO ĐẠI SỐ TUYẾN TÍNH}\\[0.6cm]

\rule{0.55\linewidth}{0.6pt}\\[2cm]

{\fontsize{22}{28}\selectfont\bfseries\color{mainblue}
FUNCTION OF MATRICES}\\[2cm]

\rule{0.55\linewidth}{0.6pt}\\[2.5cm]

\begin{tabular}{@{}ll@{}}
  \textcolor{accentred}{\textbf{Trình độ đào tạo:}}     & \textcolor{mainblue}{Thạc sĩ} \\[0.5cm]
  \textcolor{accentred}{\textbf{Học viên thực hiện:}}   & \textcolor{mainblue}{Nguyễn Hồ Bảo Thiên} \\[0.5cm]
  \textcolor{accentred}{\textbf{Lớp:}}                  & \textcolor{mainblue}{Khoa học dữ liệu K28B} \\[0.5cm]
  \textcolor{accentred}{\textbf{Giảng viên hướng dẫn:}} & \textcolor{mainblue}{TS. Nguyễn Văn Vũ} \\
\end{tabular}

\vfill

{\normalsize Gia Lai, 2026}
...
# -------------------------------------------------------
# 1. Ma tran vi du: co so dieu kien cao
# -------------------------------------------------------
eps = 1e-5
...
# -------------------------------------------------------
# 2. Phuong phap 1: Phan tich tri rieng (eigenvector)
# -------------------------------------------------------
eigenvalues, X = np.linalg.eig(A)
kappa = np.linalg.cond(X)               # so dieu kien cua X
f_eig = np.exp(eigenvalues)             # ap dung f = exp len tri rieng
fA_eig = X @ np.diag(f_eig) @ np.linalg.inv(X)

# -------------------------------------------------------
# 3. Phuong phap 2: Phan tich Schur + cong thuc phan tu
# -------------------------------------------------------
T, Q = schur(A, output='complex')       # A = Q T Q^H
# Tinh f(T) cho ma tran tam giac tren 2x2
fT = np.zeros_like(T)
lam = np.diag(T)
fT[0, 0] = np.exp(lam[0])
fT[1, 1] = np.exp(lam[1])
# Sai phan chia bac 1 cho phan tu (0,1)
if abs(lam[1] - lam[0]) > 1e-14:
    fT[0, 1] = T[0, 1] * (fT[1, 1] - fT[0, 0]) / (lam[1] - lam[0])
else:
    fT[0, 1] = T[0, 1] * np.exp(lam[0])   # gioi han khi lambda trung nhau
fA_schur = Q @ fT @ Q.conj().T

# -------------------------------------------------------
# 4. Phuong phap 3: SciPy expm (Pade + thu nho va binh phuong)
# -------------------------------------------------------
fA_scipy = expm(A)
...
# -------------------------------------------------------
print("="*55)
print(f"  So dieu kien cua ma tran vector rieng: kappa = {kappa:.2e}")
print("="*55)
print("\nKet qua qua phan tich tri rieng (eigenvector):")
print(fA_eig.real)
print("\nKet qua qua Schur-Parlett (don gian hoa):")
print(fA_schur.real)
print("\nKet qua SciPy expm (chuan xac):")
print(fA_scipy)

err_eig    = np.linalg.norm(fA_eig.real - fA_scipy, 'fro')
err_schur  = np.linalg.norm(fA_schur.real - fA_scipy, 'fro')
print(f"\nSai so Frobenius (eigenvector vs scipy): {err_eig:.2e}")
print(f"Sai so Frobenius (Schur      vs scipy): {err_schur:.2e}")
\end{lstlisting}
...
# -------------------------------------------------------
# Ham tinh e^A bang chuoi Taylor cat cut bac q
# -------------------------------------------------------
def expm_taylor(A, q):
    """Xap xi e^A bang tong Taylor bac q."""
    n = A.shape[0]
    F    = np.eye(n)          # so hang k=0
    Ak   = np.eye(n)          # A^k
    fact = 1.0
...
# Ham tinh da thuc ma tran p(A) bang luoc do Horner
def horner(A, coeffs):
    """
    Tinh p(A) = coeffs[0]*I + coeffs[1]*A + ... + coeffs[q]*A^q
    dung luoc do Horner.
    coeffs[0] la he so tu do, coeffs[-1] la he so bac cao nhat.
    """
    n = A.shape[0]
    q = len(coeffs) - 1
    F = coeffs[q] * np.eye(n)
...
# -------------------------------------------------------
# Vi du 1: Ma tran hien lanh
# A co tri rieng am (on dinh, ham mu giam)
# -------------------------------------------------------
A_ok = np.array([[1.0, 2.0],
                 [0.0, 3.0]])
ref  = expm(A_ok)

print("Vi du 1 -- Ma tran binh thuong A = [[1,2],[0,3]]")
print(f"  Ket qua chuan xac (SciPy): \n{ref}\n")
for q in [5, 10, 20, 30]:
    approx = expm_taylor(A_ok, q)
    err = np.linalg.norm(approx - ref, 'fro')
    print(f"  q={q:2d}  ||Taylor - chinh xac||_F = {err:.2e}")

# -------------------------------------------------------
# Vi du 2: Ma tran "ngo ngao" -- huy so nghiem trong
# A co tri rieng 1 va -17 (pho lon)
# -------------------------------------------------------
A_bad = np.array([[-49.0, 24.0],
                  [-64.0, 31.0]])
ref_bad = expm(A_bad)

print("\nVi du 2 -- Ma tran gay huy so A = [[-49,24],[-64,31]]")
print(f"  Ket qua chuan xac:\n{ref_bad}\n")
print("  Bac q  ||Taylor - chinh xac||_F   Max|phan tu trung gian|")
for q in [10, 20, 40, 59]:
    approx = expm_taylor(A_bad, q)
    err = np.linalg.norm(approx - ref_bad, 'fro')
    # Tinh gia tri lon nhat cua tong trung gian (theo doi huy so)
    n = A_bad.shape[0]
...
# -------------------------------------------------------
# Vi du 3: Luoc do Horner cho da thuc ma tran
# p(x) = 1 + x + x^2/2 + x^3/6 (Taylor bac 3 cua e^x)
# -------------------------------------------------------
A_small = np.array([[0.1, 0.2],
                    [0.0, 0.3]])
coeffs = [1.0, 1.0, 0.5, 1/6]   # he so [b0, b1, b2, b3]
p_horner = horner(A_small, coeffs)
p_direct = (np.eye(2) + A_small
            + A_small @ A_small / 2
            + A_small @ A_small @ A_small / 6)
print("\nVi du 3 -- Kiem tra Horner vs tinh truc tiep:")
print(f"  ||Horner - tinh truc tiep||_F = {np.linalg.norm(p_horner - p_direct):.2e}")
\end{lstlisting}
...
def expm_scaling_squaring(A, p=6):
    """
    Scaling and Squaring voi xap xi Pade bac (p,p).
    - Thu nho: chon j sao cho ||A/2^j||_inf <= 0.5
    - Tinh Pade: R = D^{-1} N
    - Binh phuong j lan
    """
    ...
    for _ in range(j):                     # binh phuong j lan
        F = F @ F
    return F

# -------------------------------------------------------
# So sanh voi SciPy expm
# -------------------------------------------------------
...
print("So sanh Scaling-Squaring (Pade p=6) vs SciPy expm:")
print(f"  ||F_mine - F_scipy||_F = {err:.2e}")

# -------------------------------------------------------
# Hien tuong tang truong qua do
# A = [[-1, 1000],[0, -1]], tri rieng deu = -1 (am)
# nhung ||e^At||_2 tang manh truoc khi giam
# -------------------------------------------------------
...
print("\nHien tuong tang truong qua do voi A = [[-1,1000],[0,-1]]:")
print(f"  Gia tri lon nhat cua ||e^At||_2 = {norms[idx_max]:.1f}")
print(f"  Dat tai t = {t_vals[idx_max]:.3f}")
print(f"  (du rang alpha(A) = -1 < 0 nen e^At phai giam trong dai han)")

# Kiem chung bang cong thuc giai tich
# e^{At} = e^{-t} [[1, 1000t],[0, 1]]
t_check = t_vals[idx_max]
eAt_analytic = np.exp(-t_check) * np.array([[1, 1000*t_check],
                                             [0, 1]])
print(f"  Kiem tra: ||e^At||_2 (giai tich) = "
...
# -------------------------------------------------------
# 1. Lap Newton cho ham dau ma tran
...
def matrix_sign_newton(A, max_iter=50, tol=1e-14):
    ...
    for k in range(max_iter):
        ...
        if residual < tol:
            print(f"  [sign] Hoi tu sau {k+1} vong lap, phan du = {residual:.2e}")
            return S_new
        S = S_new
    print(f"  [sign] Chua hoi tu sau {max_iter} vong lap")
    return S

# -------------------------------------------------------
# 2. Lap Denman-Beavers cho can bac hai ma tran
...
def matrix_sqrt_denman_beavers(A, max_iter=50, tol=1e-14):
    ...
    for k in range(max_iter):
        ...
        if err < tol:
            print(f"  [sqrt] Hoi tu sau {k+1} vong lap, ||X^2-A||_F = {err:.2e}")
            return X_new, Y_new
        X, Y = X_new, Y_new
    return X, Y

# -------------------------------------------------------
# 3. Phan tich cuc: Newton lap cho nhan tu truc giao
...
def polar_newton(A, max_iter=50, tol=1e-14):
    ...
    for k in range(max_iter):
        ...
        if err < tol:
            print(f"  [polar] Hoi tu sau {k+1} vong lap, ||U^T U - I||_F = {err:.2e}")
            return X_new
        X = X_new
    return X

# -------------------------------------------------------
# 4. Logarit ma tran: Inverse Scaling and Squaring
#    Dung can bac hai lap + Maclaurin cho log
# -------------------------------------------------------
def matrix_log_iss(A, tol_norm=0.5, q=20):
    ...
    while np.linalg.norm(Ak - np.eye(Ak.shape[0]), 'fro') > tol_norm:
        ...
        k += 1
    # Tinh log(Ak) bang chuoi Maclaurin (Ak gan I)
    ...
    for j in range(1, q + 1):
        Bk    = Bk @ B
        logA += ((-1)**(j+1) / j) * Bk
    return (2**k) * logA   # hoan nguyen

# -------------------------------------------------------
# UNG DUNG VA KIEM TRA
# -------------------------------------------------------
A = np.array([[4.0, 3.0],
              [1.0, 2.0]])
...
ref_sqrt   = sqrtm(A)
print("\nCan bac hai A^{1/2} (Denman-Beavers):")
print(X_sqrt)
print(f"  Sai so so voi sqrtm(scipy): {np.linalg.norm(X_sqrt - ref_sqrt, 'fro'):.2e}")

# Phan tich cuc
U_polar = polar_newton(A)
U_svd   = (lambda U,_,Vt: U @ Vt)(*svd(A, full_matrices=False))
print("\nNhan tu truc giao U (phan tich cuc, Newton):")
print(U_polar)
print(f"  Sai so so voi SVD: {np.linalg.norm(U_polar - U_svd, 'fro'):.2e}")

# Logarit
L = matrix_log_iss(A, tol_norm=0.5, q=30)
ref_log = logm(A)
print("\nLogarit chinh log(A) (Inverse Scaling and Squaring):")
print(L)
print(f"  Sai so so voi logm(scipy): {np.linalg.norm(L - ref_log, 'fro'):.2e}")
print(f"  Kiem tra: ||e^L - A||_F = "
...
print(f"Epsilon may u = {u:.3e}")
print(f"Phien ban NumPy: {np.__version__}")

# Ham tao ma tran ngau nhien voi so dieu kien cho truoc
...
for name, M in MATS.items():
    ...
    print(f"  {name:15s}: kich thuoc={M.shape}, "
          f"kappa={kappa:.2e}, "
    ...
    for p in range(1, n):
        for i in range(n - p):
            ...
            if abs(dl) > 1e-10:
                ...
                # Gioi han: f'(lambda) khi tri rieng trung nhau
                Fij = T[i, j] * np.exp(lam[i])
            # Dong gop tu cac phan tu trung gian
...
# -------------------------------------------------------
print(f"{'Ma tran':15s} | {'Sai so Eig':10s} | {'Sai so Schur':10s} "
...
# Cai dat lai cac ham lap voi theo doi hoi tu
...
# -------------------------------------------------------
# Ma tran thu nghiem: SPD, de kiem chung
# -------------------------------------------------------
...
# Thi nghiem: Ham dau sign(B)
print("\n--- Ham dau ma tran (Newton) ---")
...
print("  Phan du ||S^2 - I||_F theo vong lap:")
...
print("  Phan du ||X^2 - B||_F theo vong lap:")
...
\item G.~H. Golub \textbf{and} C.~F.~V. Loan,
\textit{Matrix computations},
In association with the Department of Mathematical Sciences,
The Johns Hopkins University.,
Johns Hopkins Univ. Press, Baltimore, MD, 4.~ed., 2013, 756~\textbf{pagetotals}.

\item N.~J. Higham,
\textit{Functions of Matrices},
Society for Industrial \textbf{and} Applied Mathematics, 2008.
